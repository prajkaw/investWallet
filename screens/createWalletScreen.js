...
export default class CreateWalletScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    let RB = Array.from(Random.getRandomBytes(16));
  ...
  // Method to generate mnemonic phrases
  generateMnemonic = (strength) => {
    // Strength should be passed 128 as an argument because we need 12 phrases
    ...
    rng = randomBytes;
    console.log(entropyToMnemonic(rng(strength / 8), wordList));
  };

  // // Method to convert entropy to mnemonic phrases
  entropyToMnemonic = (entropy, wordlist) => {
    // If its not buffer extract buffer from entropy
    if (!Buffer.isBuffer(entropy)) {
      entropy = Buffer.from(entropy, "hex");
    }
    // Should have word list with 2048 words
    if (!wordlist) {
      throw new Error(WORDLIST_REQUIRED);
    }
    // entropy should be in 128 <= ENT <= 256
    ...
    // entropy length should be divisible by 4
    if (entropy.length % 4 !== 0) {
      throw new TypeError(INVALID_ENTROPY);
    }
    const entropyBits = this.bytesToBinary(Array.from(entropy));
    const checksumBits = this.deriveChecksumBits(entropy);
  ...
  // Method to convert bytes to binary
  bytesToBinary = (bytes) => {
    return bytes.map((x) => this.lpad(x.toString(2), "0", 8)).join("");
  };

  // Method to do left padding
  ...
  // Method to extract checksum bits of buffer
  ...
  // Method to convert binary value to bytes
  binaryToByte = (bin) => {
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        >
          <FlatList
            data={this.state.mnemonic.split(" ")}
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (this.state.walletPassword.length > 0) this.createWallet();
          }}
        >
